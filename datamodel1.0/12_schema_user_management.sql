...
-- ============================================================================
-- 3. USERS TABLE - Main user account table with security features
-- ============================================================================
...
AND p.permission_name IN ('view_members', 'edit_members', 'view_claims', 'edit_claims', 'view_providers', 'view_users', 'view_reports', 'export_data')
ON CONFLICT (role_id, permission_id) DO NOTHING;

INSERT INTO user_mgmt.role_permissions (role_id, permission_id)
SELECT r.role_id, p.permission_id FROM user_mgmt.roles r, user_mgmt.permissions p
WHERE r.role_name = 'agent'
AND p.permission_name IN ('view_members', 'edit_members', 'view_claims', 'edit_claims', 'view_providers', 'view_users', 'view_reports')
ON CONFLICT (role_id, permission_id) DO NOTHING;

INSERT INTO user_mgmt.role_permissions (role_id, permission_id)
SELECT r.role_id, p.permission_id FROM user_mgmt.roles r, user_mgmt.permissions p
WHERE r.role_name = 'viewer'
AND p.permission_name IN ('view_members', 'view_claims', 'view_providers', 'view_reports')
ON CONFLICT (role_id, permission_id) DO NOTHING;

-- ============================================================================
-- 5. AUDIT_LOG TABLE - Comprehensive audit trail
-- ============================================================================
...
-- ============================================================================
-- 6. LOGIN_HISTORY TABLE - Track login attempts
-- ============================================================================
...
-- ============================================================================
-- 7. SESSION TABLE - Track active sessions
-- ============================================================================
...
-- ============================================================================
-- 9. SAMPLE DATA - Demo Users
-- ============================================================================
-- Password: "secret" hashed with bcrypt (cost=12)
-- Hash: $2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5YmMxSUGyohie

INSERT INTO user_mgmt.users (username, email, full_name, hashed_password, role_id, is_active, is_verified, phone_number, department)
SELECT 'admin', 'admin@healthplan.com', 'System Administrator', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5YmMxSUGyohie', r.role_id, TRUE, TRUE, '(800) 555-0100', 'IT Administration'
FROM user_mgmt.roles r WHERE r.role_name = 'admin'
ON CONFLICT (username) DO NOTHING;

INSERT INTO user_mgmt.users (username, email, full_name, hashed_password, role_id, is_active, is_verified, phone_number, department)
SELECT 'supervisor', 'supervisor@healthplan.com', 'Call Center Supervisor', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5YmMxSUGyohie', r.role_id, TRUE, TRUE, '(800) 555-0101', 'Call Center Management'
FROM user_mgmt.roles r WHERE r.role_name = 'supervisor'
ON CONFLICT (username) DO NOTHING;

INSERT INTO user_mgmt.users (username, email, full_name, hashed_password, role_id, is_active, is_verified, phone_number, department)
SELECT 'agent', 'agent@healthplan.com', 'Call Center Agent', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5YmMxSUGyohie', r.role_id, TRUE, TRUE, '(800) 555-0102', 'Call Center Operations'
FROM user_mgmt.roles r WHERE r.role_name = 'agent'
ON CONFLICT (username) DO NOTHING;

INSERT INTO user_mgmt.users (username, email, full_name, hashed_password, role_id, is_active, is_verified, phone_number, department)
SELECT 'viewer', 'viewer@healthplan.com', 'Report Viewer', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewY5YmMxSUGyohie', r.role_id, TRUE, TRUE, '(800) 555-0103', 'Finance'
...
SELECT 
    u.user_id, u.username, u.email, u.full_name, u.phone_number, u.department,
    r.role_id, r.role_name, r.permission_level, u.is_active, u.is_verified, u.is_deleted,
    u.last_login, u.last_password_change, u.mfa_enabled, u.failed_login_attempts, u.locked_until,
...
SELECT DISTINCT
    u.user_id, u.username, u.email, r.role_id, r.role_name, r.permission_level,
    p.permission_id, p.permission_name, p.resource, p.action, p.permission_level as perm_level
FROM user_mgmt.users u
JOIN user_mgmt.roles r ON u.role_id = r.role_id
...
SELECT 
    lh.login_id, u.user_id, u.username, u.email, lh.status, lh.ip_address, lh.device_info, lh.mfa_verified, lh.created_at,
...
SELECT 
    s.session_id, s.user_id, u.username, u.email, u.full_name, s.ip_address, s.device_info, s.browser_fingerprint,
    s.is_mfa_verified, s.created_at, s.expires_at, s.last_activity,
...
SELECT 
    al.audit_id, al.user_id, u.username, u.email, al.action, al.resource_type, al.resource_id, al.resource_name, al.status, al.ip_address, al.created_at,
...
BEGIN
    ...
    SET last_login = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP, failed_login_attempts = 0, locked_until = NULL
    WHERE user_id = p_user_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION user_mgmt.fn_increment_failed_login(p_user_id INT, p_max_attempts INT DEFAULT 5, p_lockout_duration INTERVAL DEFAULT '30 minutes')
...
BEGIN
    UPDATE user_mgmt.users SET failed_login_attempts = failed_login_attempts + 1 WHERE user_id = p_user_id RETURNING failed_login_attempts INTO v_attempts;
    IF v_attempts >= p_max_attempts THEN
        v_locked_until := CURRENT_TIMESTAMP + p_lockout_duration;
        UPDATE user_mgmt.users SET locked_until = v_locked_until WHERE user_id = p_user_id;
        v_is_locked := TRUE;
    END IF;
...
BEGIN
    ...
    IF v_locked_until IS NULL OR v_locked_until < CURRENT_TIMESTAMP THEN RETURN FALSE; ELSE RETURN TRUE; END IF;
END;
$$ LANGUAGE plpgsql;
...
RETURNS VOID AS $$
BEGIN
    UPDATE user_mgmt.users SET failed_login_attempts = 0, locked_until = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = p_user_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION user_mgmt.fn_create_audit_log(p_user_id INT, p_action VARCHAR, p_resource_type VARCHAR, p_resource_id INT, p_resource_name VARCHAR, p_old_values JSONB, p_new_values JSONB, p_ip_address VARCHAR, p_status VARCHAR DEFAULT 'success')
RETURNS BIGINT AS $$
DECLARE
    v_audit_id BIGINT;
BEGIN
    INSERT INTO user_mgmt.audit_logs (user_id, action, resource_type, resource_id, resource_name, old_values, new_values, ip_address, status)
    VALUES (p_user_id, p_action, p_resource_type, p_resource_id, p_resource_name, p_old_values, p_new_values, p_ip_address, p_status)
    RETURNING audit_id INTO v_audit_id;
    RETURN v_audit_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION user_mgmt.fn_create_session(p_user_id INT, p_session_id VARCHAR, p_token VARCHAR, p_ip_address VARCHAR, p_expires_at TIMESTAMP)
RETURNS VARCHAR AS $$
DECLARE
    v_session_id VARCHAR;
BEGIN
    v_session_id := COALESCE(p_session_id, gen_random_uuid()::VARCHAR);
    INSERT INTO user_mgmt.sessions (session_id, user_id, token, ip_address, expires_at) VALUES (v_session_id, p_user_id, p_token, p_ip_address, p_expires_at);
    RETURN v_session_id;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    v_count INT;
BEGIN
    DELETE FROM user_mgmt.sessions WHERE expires_at < CURRENT_TIMESTAMP AND is_active = TRUE;
...
CREATE TRIGGER tr_users_update_timestamp BEFORE UPDATE ON user_mgmt.users FOR EACH ROW EXECUTE FUNCTION user_mgmt.fn_update_timestamp();

DROP TRIGGER IF EXISTS tr_roles_update_timestamp ON user_mgmt.roles;
CREATE TRIGGER tr_roles_update_timestamp BEFORE UPDATE ON user_mgmt.roles FOR EACH ROW EXECUTE FUNCTION user_mgmt.fn_update_timestamp();
...
CREATE TRIGGER tr_roles_lowercase_name BEFORE INSERT OR UPDATE ON user_mgmt.roles FOR EACH ROW EXECUTE FUNCTION user_mgmt.fn_lowercase_role_name();
...
CREATE TRIGGER tr_session_activity BEFORE UPDATE ON user_mgmt.sessions FOR EACH ROW WHEN (OLD.last_activity IS DISTINCT FROM NEW.last_activity) EXECUTE FUNCTION user_mgmt.fn_update_session_activity();

-- ============================================================================
-- 13. CONSTRAINTS
-- ============================================================================

ALTER TABLE user_mgmt.users ADD CONSTRAINT ck_users_password_not_empty CHECK (length(hashed_password) > 0);
ALTER TABLE user_mgmt.users ADD CONSTRAINT ck_users_email_format CHECK (email ~ '^[^@]+@[^@]+\.[^@]+$');
ALTER TABLE user_mgmt.users ADD CONSTRAINT ck_users_username_length CHECK (length(username) >= 3);
...
-- ============================================================================
-- END OF USER MANAGEMENT SCHEMA (v2.0 Enhanced)
-- ============================================================================
